...
CREATE TABLE slider (
    ...
    description   TEXT,
    owner         INTEGER,
    to_collection INTEGER UNIQUE,
    to_art        INTEGER UNIQUE,
    to_sch        INTEGER UNIQUE,
    to_prv_d      INTEGER UNIQUE,
    to_prv_h      INTEGER UNIQUE,
    lt_t          TEXT[],
    lt_d          TEXT[],
    pfile         TEXT[],
    completed     BOOLEAN DEFAULT false,
    ...
    updated_at    TIMESTAMPTZ,
    FOREIGN KEY (owner) REFERENCES users(user_id) ON DELETE CASCADE,
    FOREIGN KEY (to_collection) REFERENCES collection(id) ON DELETE CASCADE,
